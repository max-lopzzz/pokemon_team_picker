...
**Architecture:** One cross-cutting extension to the existing, shared `MoveData` type (`highCritRate`, needed by both eras going forward), then six new `gen1`-prefixed modules: a generated type chart (derived from Phase 3a's already-verified modern chart, not re-transcribed), a historical-type lookup (`past_types`), three pure calculators (stats, crit chance, damage), and the orchestrator. Reuses Phase 1's nothing extra and Phase 3a's `getBaseStats`/`getMoveData` unchanged. Known approximation, documented in the spec: base stats and move types/power are taken as-is from PokeAPI's current values, not era-adjusted (some Gen 1 species got Gen VI base-stat buffs; a few moves changed type after Gen 1) — out of scope for this phase, unlike species typing which does get the `past_types` treatment via `gen1Types.ts`.
