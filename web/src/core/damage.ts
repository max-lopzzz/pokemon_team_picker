...
export function calculateDamageRange(
  ...
  // Games since Gen 2 clamp any non-immune hit to at least 1 damage, even
  // when rounding at low levels/power would otherwise compute to 0 (Gen 1
  // alone has no such floor). A true type immunity (typeEffectiveness === 0)
  // must stay {min: 0, max: 0} so hitsToKO can still resolve to Infinity
  // for genuinely impossible KOs.
